...
| VLA-JEPA | batch4, 0.53 step/s | 30K ≈ 15.8h | 0.302 | 非対応 (sync のみ) | 予測ノイジー (K サンプル平均で緩和)。40ep/30K では他ポリシーに劣る |
| FastWAM | batch4, 3.04 s/step | 15K ≈ 14.2h | 0.17 | 非対応 (**指定禁止**: 起動するが静かに壊れる) | sync + denoise 3 でタスク成功。オフライン指標 (方向一致 0.87〜0.89) は最良 |

- SmolVLA の位置づけ: **学習コスト最小 (GR00T の約 1/3、FastWAM の約 1/10、
  VLA-JEPA の約 1/12 の時間) で、実機のリアルタイム性 (RTC) を唯一問題なく
  満たした**。
